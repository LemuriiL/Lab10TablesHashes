// Copyright 2020 LemuriiL <LemuriiL@yandex.ru>
#include "Creator.hpp"
...
void Creator::set_values(const std::string& value) { values.push_back(value); }

int Creator::get_column_families_names_size() {
  return static_cast<int>(column_families_names.size());
}

int Creator::get_values_size() { return static_cast<int>(values.size()); }

inline void create_random_values(Creator* creator) {
  for (size_t i = 1; i <= static_cast<size_t>(random_int(
                              creator->get_column_families_names_size(), 30));
       ++i) {
...
void Creator::create_new_random_db(const std::string& path_to_new_db) {
  ...
  for (size_t i = 0; i < column_families_names.size(); ++i) {
    for (size_t j = 0; j < static_cast<size_t>(num_in_each_column[i]);
         ++j, ++count) {
      status = db_ptr->Put(write_options, handles[i],
                           "key" + std::to_string(count + 1), values[count]);
  ...
  for (int i = 0; i < max_len_col; ++i) {
    ...
    for (size_t j = 0; j < column_families_names.size(); ++j) {
      if (num_in_each_column[j] > i) {
        status = db_ptr->Get(rocksdb::ReadOptions(), handles[j],
