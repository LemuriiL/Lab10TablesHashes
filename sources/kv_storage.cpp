...
Storage::Storage(int& num_workers)
    : writer(num_workers / 2), reader(num_workers - num_workers / 2) {}

void Storage::check_status(const rocksdb::Status& status,
                           const string& message) {
  if (!status.ok()) throw std::runtime_error(message + status.ToString());
}
...
void Storage::create_iterators(
  ...
  for (auto& handle : handles_cf) {
    iterators_cf.emplace_back(db->NewIterator(rocksdb::ReadOptions(), handle));
  }
}
...
void Storage::check_iterators(
  ...
  for (size_t i = 1; i < iterators_cf.size(); ++i) {
    for (iterators_cf[i]->SeekToFirst(); iterators_cf[i]->Valid();
...
void Storage::hashing(Storage* storage, std::queue<Field>& fields,
  ...
  while (!fields.empty() || !hashs.empty()) {
    if (!fields.empty()) {
      hashs.emplace(storage->use_writer().hash256(fields.front().key,
                                                  fields.front().value),
                    fields.front().key);
      fields.pop();
    }
...
std::stringstream Storage::print_table(
  ...
  for (size_t i = 0; i < handles_cf_out.size(); ++i) {
    ...
    for (int j = 0; j < nums_in_columns[i]; ++j, ++num) {
      str_of_separator(ss);
      ss << std::setw(spases_for_table[1])
         << "| " + out_fields[num].key + " : " + out_fields[num].value << " |"
         << endl;
    }
    str_of_separator(ss);
    ss << endl << endl;
...
void Storage::start(const string& path_to_db, int num_workers,
                    const string& path_to_out_db) {
  try {
    Storage storage(num_workers);
    std::vector<string> names_cf;
    rocksdb::Status status = rocksdb::DB::ListColumnFamilies(
        rocksdb::DBOptions(), path_to_db, &names_cf);
    ...
    status = rocksdb::DB::Open(rocksdb::DBOptions(), path_to_db, descriptors_cf,
                               &handles_cf, &db);
    ...
    BOOST_LOG_TRIVIAL(info)
        << "Look to your table:\n\n"
        << Storage::print_table(out_fields, handles_cf_out, nums_in_columns)
               .str();
